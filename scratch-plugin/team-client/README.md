...
- Terminal Git synchronization: after a user watches a repository, the Client installs a repository-specific proxy without replacing the effective hook. Events enter a durable local queue; one consumer uploads the same commit summary fields and retains failed batches for retry in `src/git-hooks-sync.ts`.
