...
- Git synchronization (`src/git-sync.ts`): importing a project (`POST /team/git/import` with a directory path) runs `git log` to extract its full commit history and uploads it in batches. The Client re-scans all imported repositories on startup and every `TEAM_GIT_SCAN_MINUTES` (default 5), uploading only commits after the last-synced hash (a per-repository cursor persisted in `watched.json`; the Server deduplicates by `(git_remote, commit_hash)`). No hooks or tool-event listeners are installed: `git log` is the only data source.
