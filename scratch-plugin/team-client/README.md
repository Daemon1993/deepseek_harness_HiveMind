...
- Git synchronization (`src/git-sync.ts`): importing a project (`POST /team/git/import` with a directory path) runs `git log` and uploads author, subject, full message, changed-file paths, and line stats in batches of 100. The Client re-scans all imported repositories on startup and every `TEAM_GIT_SCAN_MINUTES` (default 5), uploading only commits after the last-synced hash (a per-repository cursor in `watched.json`; the Server deduplicates by `(git_remote, commit_hash)`). Remove an import with `POST /team/git/remove`. `GET /team/git/status` reports scan progress. No hooks or tool-event listeners are installed: `git log` is the only data source.
