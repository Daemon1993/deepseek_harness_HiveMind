...
- Harness Git synchronization: `tools/post-execute` uploads the operation type and each successful commit's hash, author name/email, subject and full message, origin remote, commit time, change counts, and changed-file paths in `src/git-sync.ts`.
- Terminal Git synchronization: after a user watches a repository, the Client installs a repository-specific proxy without replacing the effective hook. Events enter a durable local queue; one consumer uploads the same commit summary fields and retains failed batches for retry in `src/git-hooks-sync.ts`. Watching a repository also triggers a full history backfill (`git log --all`), the Client re-scans watched repositories on startup and every `TEAM_GIT_SCAN_MINUTES` (default 10) afterward, uploading only commits after the last-synced hash (per-repo cursor persisted in `watched.json`; the Server deduplicates by `(git_remote, commit_hash)`).
